...
let scores = new Map();
let playerQueue = []; // Queue for players waiting to join canvas
let queuedSockets = new Map(); // Map of socket.id to queued player data
...
io.on('connection', (socket) => {
    ...
    socket.on('join', (data) => {
        ...
        // Check if canvas is full and player needs to be queued
        if (players.size >= MAX_ACTIVE_PLAYERS) {
            console.log(`🔄 Canvas full (${players.size}/${MAX_ACTIVE_PLAYERS}). Adding ${name} to queue.`);
            addToQueue(socket, { name, color });
            return;
        }
    ...
    socket.on('disconnect', () => {
        ...
        if (player) {
            ...
            // Promote next player from queue if there's space now
            setTimeout(() => promoteFromQueue(), 500);
        }

        // Handle queued player disconnect
        if (queuedSockets.has(socket.id)) {
...
function addToQueue(socket, playerData) {
    const queueEntry = {
        socketId: socket.id,
    ...
    queuedSockets.set(socket.id, queueEntry);
    
    console.log(`🔄 Player ${playerData.name} added to queue. Position: ${playerQueue.length}`);
    
    // Send queue status to the player
    sendQueueStatus(socket);
    
    // Broadcast updated queue info to all queued players
    broadcastQueueUpdates();
}

function removeFromQueue(socketId) {
    const queueEntry = queuedSockets.get(socketId);
    if (queueEntry) {
        const index = playerQueue.findIndex(entry => entry.socketId === socketId);
        if (index !== -1) {
            playerQueue.splice(index, 1);
            queuedSockets.delete(socketId);
            console.log(`🔄 Player ${queueEntry.name} removed from queue`);
...
function promoteFromQueue() {
    if (playerQueue.length > 0 && players.size < MAX_ACTIVE_PLAYERS) {
        const nextPlayer = playerQueue.shift();
        const socket = io.sockets.sockets.get(nextPlayer.socketId);
        
        if (socket) {
            queuedSockets.delete(nextPlayer.socketId);
            
            // Add to active players
            players.set(nextPlayer.socketId, { 
                name: nextPlayer.name, 
                color: nextPlayer.color 
            });
            scores.set(nextPlayer.name, { 
                score: 0, 
                color: nextPlayer.color 
            });
            
            // Send them the game state
            socket.emit('promotedFromQueue');
            socket.emit('fullBoard', gameBoard);
            updateScores();
            io.emit('playerListUpdate', Array.from(players.values()));
            io.emit('playerJoined', { playerName: nextPlayer.name });
            
            console.log(`✅ Player ${nextPlayer.name} promoted from queue to active play`);
            
            // Update queue positions for remaining players
            broadcastQueueUpdates();
            
            // Check if we can promote more players
            if (playerQueue.length > 0 && players.size < MAX_ACTIVE_PLAYERS) {
                setTimeout(() => promoteFromQueue(), 1000); // Small delay to prevent spam
            }
        } else {
            // Socket disconnected, try next player
            promoteFromQueue();
        }
    }
}

function sendQueueStatus(socket) {
    const queueEntry = queuedSockets.get(socket.id);
    if (queueEntry) {
        const position = playerQueue.findIndex(entry => entry.socketId === socket.id) + 1;
        const estimatedWaitTime = calculateEstimatedWaitTime(position);
        
        socket.emit('queueStatus', {
            position: position,
            totalInQueue: playerQueue.length,
            activePlayerCount: players.size,
            maxActiveUsers: MAX_ACTIVE_PLAYERS,
...
function calculateEstimatedWaitTime(position) {
    // Simple estimation: assume 5 minutes average session time per player
    const averageSessionMinutes = 5;
    const playersAhead = position - 1;
    const slotsNeeded = Math.max(1, playersAhead - (MAX_ACTIVE_PLAYERS - players.size));
    return Math.max(1, Math.ceil(slotsNeeded * averageSessionMinutes / MAX_ACTIVE_PLAYERS));
}
